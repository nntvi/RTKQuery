...
type ErrorForm = { [key in keyof typeof initialState]: string } | null
...
export default function CreatePost() {
  ...
  const formError: ErrorForm = useMemo(() => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (postId) {
        ...
        // không dùng unwrap là ko có nhảy zô catch được đâu
      } else {
      ...
      <div className='mb-6'>
        <label
          ...
          className={classNames('mb-2 block text-sm font-medium  dark:text-gray-300', {
            'text-red-700': Boolean(formError?.publishDate),
            'text-gray-900': !Boolean(formError?.publishDate)
          })}
        >
          Publish Date
        </label>
        <input
          ...
          className={classNames('block w-56 rounded-lg border  p-2.5 text-sm', {
            ...
            'bg-gray-50text-gray-900 border-gray-300 focus:border-blue-500 focus:outline-none focus:ring-blue-500':
              !Boolean(formError?.publishDate)
          })}
          placeholder='Title'
          required
        ...
        />
        {formError?.publishDate && <p className='mt-2 text-sm text-red-600'>{formError?.publishDate}</p>}
      </div>
